/*
 * main.cpp - Fixed: separate RB callbacks, correct off-CPU probe attachment
 */
...
static void on_derived_metrics(const DerivedFunctionMetrics &metrics)
{
    ...
    if (g_oncpu_folded.is_open() || g_offcpu_folded.is_open()) {
        uint64_t on_us  = metrics.on_cpu_ns        / 1000;
        uint64_t off_us = metrics.off_cpu_total_ns / 1000;
        if (g_oncpu_folded.is_open())
            g_oncpu_folded  << "root;" << fname << " " << on_us  << "\n";
        if (g_offcpu_folded.is_open())
            g_offcpu_folded << "root;" << fname << " " << off_us << "\n";
    }
...
/* ═══════════════════════════════════════════════════════════════════════
 *  BUG FIX #5: Separate ring buffer callbacks per source.
 *
 *  Previously a single handle_event tried to guess the event type by
 *  peeking at raw bytes — an off_cpu_event with a large timestamp could
 *  have bytes[12] == 1 or 2 and get misrouted as a profiler_event.
 *
 *  Solution: each ring buffer gets its own callback that knows exactly
 *  what struct to expect.
 * ═══════════════════════════════════════════════════════════════════════*/
...
static void cleanup()
{
    ...
    delete g_derivation; g_derivation = nullptr;

    ring_buffer__free(g_uprobe_rb);
    ring_buffer__free(g_offcpu_rb);
    ring_buffer__free(g_oncpu_rb);

    bpf_object__close(g_oncpu_obj);
    bpf_object__close(g_uprobe_obj);
    bpf_object__close(g_offcpu_obj);
...
{
    fprintf(stderr,
        "Usage: %s <process_name> --funcs \"f1,f2,...\"\n"
        "Example: sudo %s redis-server --funcs \"processCommand,lookupCommand\"\n",
        prog, prog);
}
...
int main(int argc, char **argv)
{
    ...
    printf("\n[+] All ring buffers ready. Profiling started.\n\n");

    /* ── Web server ── */
    web_thread = std::thread([]() {
        svr.Get("/", [](const httplib::Request&, httplib::Response &res) {
            std::ifstream f("www/index.html");
            std::string html = f ? std::string(std::istreambuf_iterator<char>(f),
                                               std::istreambuf_iterator<char>())
                                 : "<h1>www/index.html not found</h1>";
        ...
        svr.Post("/control", [](const httplib::Request &req, httplib::Response &res) {
            ...
            if      (cmd == "pause")  { g_paused = true;  res.set_content(R"({"status":"paused"})",  "application/json"); }
            else if (cmd == "resume") { g_paused = false; res.set_content(R"({"status":"resumed"})", "application/json"); }
            else { res.status = 400; res.set_content(R"({"error":"unknown"})", "application/json"); }
        });

        printf("[+] Web UI → http://localhost:9000\n");
    ...
    while (g_running) {
        ...
        if (now - last_print >= 1) {
            ...
            live_data["offcpu_events"] = g_total_off_cpu_events;
            ...
            live_data["binary"]        = g_binary_path.substr(
                                             g_binary_path.find_last_of('/') + 1);
            last_print = now;
        }
    }
